...
case class Drone( position: Position = Position( Coordinate( INIT_DRONE_POSITION_X, INIT_DRONE_POSITION_Y ), INIT_DRONE_DIRECTION ) ) {
  ...
  private[entities] def moveForward(): Drone = {
    val ONE_STEP = 1
  ...
  def doInstruction( instruction: Char ): Either[ServiceError, Drone] = {
    instruction match {
      ...
      case _ => Left( ServiceError( Business, s"Invalid instruction: ${instruction}" ) )
    }
  }
